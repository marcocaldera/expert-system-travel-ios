...
(deftemplate TRIP::trip
   (multislot resort-sequence (type SYMBOL))
   (multislot place-sequence (type SYMBOL))
   (multislot certainties (type FLOAT))
   (multislot days-distribution (type INTEGER))
   (multislot price-per-night (type INTEGER))
...
(defrule BUILDSOLUTION::find-certainty
    ?p <- (trip (certainties $?certainties) (days-distribution $?days-distribution))
    (test (> (length$ ?certainties) 1))
    =>
    ...
    (if (> ?trip-cf 0.2) then (modify ?p (certainties ?trip-cf)) else (retract ?p))

)

;entra solo quanto ?Certainty contiene un solo elemento
(defrule BUILDSOLUTION::distance-penalty
    ?p <- (trip (certainties ?certainty) (place-sequence $?place-sequence) (penalty FALSE))
    ; (test (eq (length$ ?certainty) 1))
    =>
...
(deffacts RULES::expert-rules
    ...
    (rule
        ...
        (then tourism-type is balneare with certainty 0.6 and
            tourism-type is montano with certainty 0.4 and
            tourism-type is enogastronomico with certainty 0.3 and
